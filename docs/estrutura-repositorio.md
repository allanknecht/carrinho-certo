...
- **HTTP API** — endpoints for auth, receipt submission, and (planned) receipts read, products, lists.
- **Background work** — `ProcessReceiptJob` (Active Job): fetches NFC-e consultation URL, parses XML/HTML (`app/services/nfce_consultation_parser.rb`), writes `stores`, `receipts`, `receipt_items_raw`.
